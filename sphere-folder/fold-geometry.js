...
const GeometryUtils = {
  ...
  createSegments(points, folds) {
    ...
    for (let i = 0; i < points.length; i++) {
      ...
      const nearFold = folds.some(
        f => this.isPointNearFoldLine(points[i], f) || this.isPointNearFoldLine(points[nextIdx], f)
      );
  ...
  findEnclosedRegions(points, folds) {
    ...
    segments.forEach(seg => {
      if (seg.length >= 3) {
        const adjacentFolds = folds.filter(f => seg.some(p => this.isPointNearFoldLine(p, f, 15)));
  ...
  applyFoldTransforms(vertices, face, faceIndex) {
    ...
    face.adjacentFolds.forEach(f => {
      if (f.angle && f.angle !== 0) {
        ...
        const axis = new THREE.Vector3(f[1].x - f[0].x, f[1].y - f[0].y, 0).normalize();
  ...
  foldNet(points, folds, faceIndexOffset = 0) {
    ...
    faces.forEach((face, fIdx) => {
      ...
      const folded = this.applyFoldTransforms(verts2d, faceForFold, faceIndexOffset + fIdx);
