class CurveFolderApp {
  constructor() {
    ...
    this.detectFaces = () => {
      this.faces = this.findEnclosedRegions(this.points, this.folds);
    };
    // -------------------------------------------------------
    ...
    switch (this.mode) {
      case 'draw':
        if (!this.isClosed) {
          instructions.textContent =
            'Click to add points to create a piecewise-linear curve. Click "Close Curve" when done.';
        } else {
      ...
      case 'fold':
        if (!this.isClosed) {
          ...
          instructions.textContent =
            'Click two points on the curve to define a fold line, or click existing fold lines to set angles. Click "Fold Up!" when ready.';
        }
        break;
      case 'view':
  ...
  handleFoldClick(x, y) {
    ...
    for (let i = 0; i < this.folds.length; i++) {
      ...
      if (fold.length === 2) {
        const distToFold = this.distanceToLineSegment(
          x,
          y,
          fold[0].x,
          fold[0].y,
  ...
  createSegments() {
    ...
    for (let i = 0; i < this.points.length; i++) {
      ...
      const isNearFold = this.folds.some(fold => {
        return (
          this.isPointNearFoldLine(this.points[i], fold) ||
  ...
  detectFaces() {
    ...
    for (let i = 0; i < this.points.length; i++) {
      ...
      if (!vertices.has(key1)) {
        vertices.set(key1, p1);
      }
      if (!vertices.has(key2)) {
        vertices.set(key2, p2);
      }
    }
    ...
    this.folds.forEach(fold => {
      if (fold.length === 2) {
        ...
        if (!vertices.has(key1)) {
          vertices.set(key1, fold[0]);
        }
        if (!vertices.has(key2)) {
          vertices.set(key2, fold[1]);
        }
      }
    });
  ...
  addWireframeEdges(scale, centerX, centerY) {
    ...
    this.folds.forEach(fold => {
      if (fold.length === 2) {
        ...
        const foldVertices = [
          (fold[0].x - centerX) * scale,
          -(fold[0].y - centerY) * scale,
          0,
          (fold[1].x - centerX) * scale,
          -(fold[1].y - centerY) * scale,
          0,
        ];
